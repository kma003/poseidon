...
df_train = pd.DataFrame(list(zip(image_filenames_train,annotation_filenames_train)), columns = ['image_filename','annoation_filename'])

# Create validation dataframe
image_filenames_val = [f+".png" for f in base_filenames_val]
annotation_filenames_val = [f+".txt" for f in base_filenames_val]
df_val = pd.DataFrame(list(zip(image_filenames_val,annotation_filenames_val)), columns = ['image_filename','annoation_filename'])
...
df_test = pd.DataFrame(list(zip(image_filenames_test,annotation_filenames_test)), columns = ['image_filename','annoation_filename'])
